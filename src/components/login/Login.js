...
const Login = () => {
  ...
  useEffect(() => {
    if (!input11.current.value && firstInput && accCreation && init) {
      ...
      if(input11.current.value.length < 3) {
        ...
        i1a.current.style.opacity = '1'
        i1a.current.style.transform = 'translateY(0rem)'
        inputHtml.current.textContent = 'Please enter a username that is at least 3 characters long.'
      } else if(input11.current.value.length > 16) {
        inputHtml.current.textContent = 'Please enter a username that is not longer than 16 characters.'
        input11.current.style.borderBottom = '1px solid #d93025'
        i1a.current.style.opacity = '1'
        i1a.current.style.transform = 'translateY(0rem)'
      }
  ...
  useEffect(() => {
    if(accCreation) {
      ...
      accCreationBtn.current.addEventListener('mouseenter', accCreationSignInME)
      accCreationBtn.current.addEventListener('mouseleave', accCreationSignInML)
      ...
      .then(data => {
        for( let userId in data) {
          ...
          if(data[userId].username === usernameValue && data[userId].password === passwordValue){
            ...
            ul1.current.childNodes.forEach(li => li.style.transform = 'translateY(17rem)')
            ...
            i1a.current.style.opacity = '1'
            i1a.current.style.transform = 'translateY(0rem)'
            inputHtml.current.textContent = 'The login attempt was unsuccessful.'
          }
        }
  ...
  const sendToFirebase = async () => {   
      ...
      .then(data => {
        for( let userId in data) {
          ...
          if(data[userId].username === usernameValue){
            ...
            i1a.current.style.opacity = '1'
            i1a.current.style.transform = 'translateY(0rem)'
            inputHtml.current.textContent = 'This username is taken, attempt was unsuccessful.'
    ...
    if(usernameSafe && passwordSafe && !same) {
      alert(same)
      const data = { username: input11.current.value, password: input21.current.value };
      fetch('https://contenttime-90543.firebaseio.com/auth.json', {
        method: 'POST',
        headers: {
      ...
      .then(response => {
        response.json()
        i1a.current.style.opacity = 1
        inputHtml.current.textContent = 'Your account has been created successfully.'
        setFinished(true)
      })
      .then(data => {
        console.log('Success:', data);
  ...
  const checkInput = (e) => {
    ...
    if(e.length >= 8) {
      ...
      if(lcBool && ucBool && numberBool){
        ...
        inputHtml.current.textContent = `Please enter a password that is at least 8 characters long, containing a minimum of ${ucBool ? '' : '1 uppercase,'} ${lcBool ? '' : '1 lowercase,'} ${numberBool ? '' : '1 numeric character'}`
        i1a.current.style.opacity = '1'
        i1a.current.style.transform = 'translateY(-2rem)'
      ...
      inputHtml.current.textContent = 'Please enter a password that is at least 8 characters long, containing a minimum of 1 uppercase, 1 lowercase and 1 numeric character.'
      i1a.current.style.opacity = '1'
      i1a.current.style.transform = 'translateY(-2rem)'
      input21.current.style.borderBottom = '1px solid #d93025'
      setpasswordSafe(false)
